...
export async function checkWAClientHealth() {
  ...
  try {
    ...
    // Frame/browser crash detected during health check itself — reset immediately
    console.error("[BotTorik] Health check gagal (frame crash?):", err.message);
    resetWAClient();
    return false;
  }
}

// ── Periodic ping: detect frame crashes early and auto-reset ─────────────────
let _pingInterval = null;

function startClientPing() {
  if (_pingInterval) return; // already running
  _pingInterval = setInterval(async () => {
    if (!global._waClient || global._waStatus !== "ready") return;
    try {
      const state = await global._waClient.getState();
      if (state !== "CONNECTED") {
        console.warn("[BotTorik] Ping: state tidak CONNECTED (" + state + ") — mereset client");
        resetWAClient();
      }
    } catch (err) {
      console.error("[BotTorik] Ping: frame crash terdeteksi —", err.message, "— mereset client");
      resetWAClient();
    }
  }, 30_000);
  // Don't block process exit
  if (_pingInterval.unref) _pingInterval.unref();
  console.log("[BotTorik] Periodic ping dimulai (setiap 30 detik)");
}
...
function stopClientPing() {
  if (_pingInterval) {
    ...
    _pingInterval = null;
    console.log("[BotTorik] Periodic ping dihentikan");
  }
}
...
export async function initWhatsAppClient() {
  ...
  global._waClient = new Client({
    ...
    puppeteer: {
      ...
      args: [
        ...
        "--no-first-run",
        "--no-zygote",
        // Removed --single-process: it collapses renderer into the browser
        // process which makes frame crashes unrecoverable
        "--disable-gpu",
        ...
        "--disable-sync-xhr",
        // Prevent aggressive memory cleanup that kills idle frames
        "--memory-pressure-off",
        // Keep renderer active even when the tab is backgrounded
        "--disable-renderer-backgrounding",
        // Prevent the hang monitor from killing frames under load
  ...
  global._waClient.on("disconnected", (reason) => {
    console.warn("[BotTorik] WhatsApp terputus:", reason, "— akan reinisialisasi dalam 5 detik...");
    stopClientPing();
    resetWAClient();
    // Auto-reinitialize after a short delay so the next send/ping finds a live client
    setTimeout(() => {
      console.log("[BotTorik] Memulai ulang client WA setelah disconnect...");
      initWhatsAppClient().catch((err) => {
        console.error("[BotTorik] Gagal reinisialisasi setelah disconnect:", err.message);
      });
    }, 5_000);
  });
...
export async function sendWhatsAppMessage(to, text) {
  // ── Pre-flight: if client is null/disconnected, attempt auto-reinit ────────
  if (!global._waClient || global._waStatus !== "ready") {
    console.warn("[BotTorik] Client WA tidak siap — mencoba reinisialisasi...");
    try {
      await initWhatsAppClient();
      // Give the client a moment to reach ready state after initialize()
      await new Promise((r) => setTimeout(r, 3_000));
    } catch (err) {
      console.error("[BotTorik] Reinisialisasi gagal:", err.message);
    }
    // If still not ready after reinit attempt, bail out
    if (!global._waClient || global._waStatus !== "ready") {
      console.error("[BotTorik] Client WA tetap tidak siap setelah reinisialisasi — batalkan pengiriman");
      return false;
    }
  }
  ...
  if (!healthy) {
    // Health check itself may have triggered a reset; try one reinit before giving up
    console.warn("[BotTorik] Health check gagal — mencoba reinisialisasi sebelum kirim...");
    try {
      await initWhatsAppClient();
      await new Promise((r) => setTimeout(r, 3_000));
    } catch (err) {
      console.error("[BotTorik] Reinisialisasi setelah health check gagal:", err.message);
    }
    if (!global._waClient || global._waStatus !== "ready") {
      console.error("[BotTorik] Client WA tetap tidak siap — batalkan pengiriman");
      return false;
    }
  }
